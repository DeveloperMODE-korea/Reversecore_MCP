...
async def trinity_defense(
    ...
    try:
        if generate_vaccine:
            for i, threat in enumerate(refined_threats):
                if ctx:
                    await ctx.info(f"  [{i+1}/{len(refined_threats)}] Generating defense for {threat.get('function', threat.get('address'))}...")
    ...
    if ctx:
        ...
        await ctx.info(f"📊 Summary: {len(all_threats)} threats discovered, {len(refined_threats)} analyzed, {len(defenses)} defenses generated")
