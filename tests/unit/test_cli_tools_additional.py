...
def _mock_path(tmp_path, name: str):
    mocked = tmp_path / name
    mocked.write_text("stub")
    return mocked


def test_run_file_success(monkeypatch, tmp_path):
    mocked_path = _mock_path(tmp_path, "x")
    monkeypatch.setattr(
        cli_tools, "validate_file_path", lambda p, read_only=False: mocked_path
    )
    monkeypatch.setattr(
        cli_tools,
...
def test_run_binwalk_success(monkeypatch, tmp_path):
    mocked_path = _mock_path(tmp_path, "fw.bin")
    monkeypatch.setattr(
        cli_tools, "validate_file_path", lambda p, read_only=False: mocked_path
    )
    monkeypatch.setattr(
...
def test_run_radare2_success(monkeypatch, tmp_path):
    mocked_path = _mock_path(tmp_path, "a.out")
    monkeypatch.setattr(
        cli_tools, "validate_file_path", lambda p, read_only=False: mocked_path
    )
