...
class TestNeuralDecompileFunction:
    ...
    async def test_neural_decompile_ghidra_not_available(self, sample_binary_path, patched_config):
        ...
        with patch(
            "reversecore_mcp.tools.neural_decompiler.ensure_ghidra_available", return_value=False
        ):
            with patch(
                "reversecore_mcp.tools.r2_analysis._execute_r2_command", new_callable=AsyncMock
            ) as mock_r2:
                mock_r2.return_value = ("int main() { return 0; }", 100)

                result = await neural_decompile(str(sample_binary_path), "0x1000", use_ghidra=True)
    ...
    async def test_neural_decompile_radare2_only(self, sample_binary_path, patched_config):
        ...
        with patch(
            ...
            result = await neural_decompile(str(sample_binary_path), "main", use_ghidra=False)
...
class TestDiffBinaries:
    ...
    async def test_diff_binaries_with_mocked_r2(self, workspace_dir, patched_config):
        ...
        with patch(
            "reversecore_mcp.tools.diff_tools._execute_r2_command", new_callable=AsyncMock
        ) as mock_r2:
