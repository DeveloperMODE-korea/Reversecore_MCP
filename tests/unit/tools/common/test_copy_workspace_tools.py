...
class TestCopyToWorkspace:
    ...
    def test_copy_to_workspace_success(self, workspace_dir, tmp_path, patched_config):
        ...
        # Copy to workspace
        result = file_operations.copy_to_workspace(str(source_file))
    ...
    def test_copy_to_workspace_custom_name(self, workspace_dir, tmp_path, patched_config):
        """Test copying file with custom destination name."""
        source_file = tmp_path / "source" / "original.bin"
        source_file.parent.mkdir()
        source_file.write_bytes(b"content")

        result = file_operations.copy_to_workspace(
    ...
    def test_copy_to_workspace_nonexistent_file(self, tmp_path, patched_config):
        ...
        result = file_operations.copy_to_workspace(str(nonexistent))
    ...
    def test_copy_to_workspace_directory(self, tmp_path, patched_config):
        ...
        result = file_operations.copy_to_workspace(str(source_dir))

        assert result.status == "error"
        assert result.error_code == "VALIDATION_ERROR"
        assert "not a file" in result.message

    def test_copy_to_workspace_file_exists(self, workspace_dir, tmp_path, patched_config):
    ...
    def test_copy_to_workspace_expanduser(self, workspace_dir, tmp_path, patched_config):
