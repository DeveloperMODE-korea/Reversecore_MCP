...
class TestCommandSpec:
    ...
    def test_command_spec_validate_match(self):
        ...
        spec = CommandSpec(name="test", type="read", regex=re.compile(r"^test(\s+\w+)?$"))
    ...
    def test_command_spec_validate_no_match(self):
        ...
        spec = CommandSpec(name="test", type="read", regex=re.compile(r"^test$"))
...
class TestDangerousPatterns:
    ...
    def test_pipe_detection(self):
        ...
        pattern = next(
            p for p in DANGEROUS_PATTERNS if "\\|" in p.pattern and "&&" not in p.pattern
        )
