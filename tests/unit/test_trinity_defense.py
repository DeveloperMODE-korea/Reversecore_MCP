...
class TestTrinityDefense:
    ...
    async def test_discover_mode(self):
        ...
        with (
            patch("reversecore_mcp.tools.trinity_defense.validate_file_path") as mock_validate,
            patch(
                "reversecore_mcp.tools.trinity_defense.ghost_trace", new_callable=AsyncMock
            ) as mock_ghost,
        ):
            ...
            result = await trinity_defense(file_path="/app/workspace/test.exe", mode="discover")
    ...
    async def test_analyze_mode(self):
        ...
        with (
            ...
            patch(
                "reversecore_mcp.tools.trinity_defense.neural_decompile", new_callable=AsyncMock
            ) as mock_neural,
        ):
            mock_validate.return_value = Path("/app/workspace/test.exe")
            ...
            mock_neural.return_value = success({"intent": "malicious", "confidence": 0.95})

            result = await trinity_defense(
                file_path="/app/workspace/test.exe", mode="analyze", max_threats=1
            )
    ...
    async def test_full_mode_with_vaccine(self):
        ...
        with (
            ...
            patch(
                "reversecore_mcp.tools.trinity_defense.adaptive_vaccine", new_callable=AsyncMock
            ) as mock_vaccine,
        ):
            mock_validate.return_value = Path("/app/workspace/test.exe")
            ...
            mock_vaccine.return_value = success({"yara_rule": "rule test { condition: true }"})
    ...
    async def test_no_threats_found(self):
        ...
        with (
            ...
            mock_ghost.return_value = success({"orphan_functions": [], "suspicious_logic": []})

            result = await trinity_defense(
                file_path="/app/workspace/test.exe", mode="full", max_threats=5
            )
    ...
    async def test_max_threats_limit(self):
        ...
        with (
            ...
            mock_ghost.return_value = success({"orphan_functions": threats, "suspicious_logic": []})

            # Mock neural decompiler
            mock_neural.return_value = success({"intent": "unknown", "confidence": 0.5})

            result = await trinity_defense(
                file_path="/app/workspace/test.exe", mode="analyze", max_threats=3
            )
    ...
    async def test_with_context_logging(self):
        ...
        with (
            ...
            result = await trinity_defense(
                file_path="/app/workspace/test.exe", mode="discover", ctx=mock_ctx
            )
    ...
    async def test_invalid_file_path(self):
        ...
        with patch("reversecore_mcp.tools.trinity_defense.validate_file_path") as mock_validate:
            ...
            result = await trinity_defense(file_path="/invalid/path", mode="discover")
    ...
    async def test_vaccine_generation_failure(self):
        ...
        with (
            ...
            mock_neural.return_value = success(
                {"intent": "unknown", "confidence": 0.5, "refined_code": "void func() {}"}
            )
