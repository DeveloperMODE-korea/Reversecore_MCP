...
class TestGhidraDecompile:
    ...
    async def test_smart_decompile_ghidra_disabled(self, workspace_dir, patched_workspace_config):
        """Test decompilation with Ghidra explicitly disabled."""
        test_file = workspace_dir / "test.exe"
        test_file.write_bytes(b"FAKE_BINARY")

        # Mock radare2 execution (r2_helpers is where execute_subprocess_async is used)
    ...
    async def test_smart_decompile_pyghidra_not_installed(
        ...
        # Simulate ImportError when trying to import ghidra
        # Mock r2_helpers where execute_subprocess_async is used
        with patch(
            "reversecore_mcp.core.r2_helpers.execute_subprocess_async",
            new_callable=AsyncMock,
            return_value=(mock_r2_output, len(mock_r2_output)),
