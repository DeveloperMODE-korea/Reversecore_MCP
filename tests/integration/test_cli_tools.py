...
class TestRunFile:
    ...
    @pytest.mark.asyncio
    async def test_run_file_success(self, sample_binary_path, patched_workspace_config):
    ...
    @pytest.mark.asyncio
    async def test_run_file_nonexistent(self, workspace_dir, patched_workspace_config):
        """Test file command on nonexistent file."""
        result = await file_operations.run_file(str(workspace_dir / "nonexistent.txt"))
    ...
    async def test_run_file_outside_workspace(
        ...
        result = await file_operations.run_file(str(outside_file))
...
class TestRunStrings:
    ...
    @pytest.mark.asyncio
    async def test_run_strings_success(self, sample_binary_path, patched_workspace_config):
        """Test successful string extraction."""
        _require_command("strings")
        result = await static_analysis.run_strings(str(sample_binary_path), min_length=4)
        assert result.status == "success"
    ...
    async def test_run_strings_min_length(self, sample_binary_path, patched_workspace_config):
        ...
        _require_command("strings")
        result = await static_analysis.run_strings(str(sample_binary_path), min_length=10)
        assert result.status == "success"
        assert "Hello World" in result.data

    @pytest.mark.asyncio
    async def test_run_strings_nonexistent(self, workspace_dir, patched_workspace_config):
        """Test strings command on nonexistent file."""
        result = await static_analysis.run_strings(str(workspace_dir / "nonexistent.txt"))
        assert result.status == "error"
        assert result.error_code == "VALIDATION_ERROR"
...
class TestRunRadare2:
    ...
    @pytest.mark.asyncio
    async def test_run_radare2_success(self, sample_binary_path, patched_workspace_config):
        ...
        result = await r2_analysis.run_radare2(str(sample_binary_path), "i")
        assert result.status == "success"
        assert isinstance(result.data, str)
        assert result.metadata and result.metadata.get("bytes_read") is not None
    ...
    async def test_run_radare2_invalid_command(self, sample_binary_path, patched_workspace_config):
        ...
        result = await r2_analysis.run_radare2(str(sample_binary_path), "invalid_command_xyz")
        assert result.status == "error"
        assert result.error_code == "VALIDATION_ERROR"
