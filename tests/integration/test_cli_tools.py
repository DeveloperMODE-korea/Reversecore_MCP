...
class TestRunFile:
    ...
    async def test_run_file_success(self, sample_binary_path, patched_workspace_config):
        ...
        result = await cli_tools.run_file(str(sample_binary_path))
    ...
    async def test_run_file_nonexistent(self, workspace_dir, patched_workspace_config):
        ...
        result = await cli_tools.run_file(str(workspace_dir / "nonexistent.txt"))
...
class TestRunStrings:
    ...
    async def test_run_strings_success(self, sample_binary_path, patched_workspace_config):
        ...
        result = await cli_tools.run_strings(str(sample_binary_path), min_length=4)
        assert result.status == "success"
    ...
    async def test_run_strings_min_length(self, sample_binary_path, patched_workspace_config):
        ...
        _require_command("strings")
        result = await cli_tools.run_strings(str(sample_binary_path), min_length=10)
        assert result.status == "success"
        assert "Hello World" in result.data
    ...
    async def test_run_strings_nonexistent(self, workspace_dir, patched_workspace_config):
        ...
        result = await cli_tools.run_strings(str(workspace_dir / "nonexistent.txt"))
        assert result.status == "error"
        assert result.error_code == "VALIDATION_ERROR"
